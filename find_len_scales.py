...
def ks_loss_logspace(log_lengths, x_known, y_known, e_known, sigma_vals, log_lower_bounds, log_upper_bounds):

    """
    KS loss wrapper for log-lengths.
    """
    lengths = np.exp(log_lengths)  # back-transform to real space

    return ks_loss(lengths, x_known, y_known, e_known, sigma_vals,
                   np.exp(log_lower_bounds), np.exp(log_upper_bounds))

####################################################################################################################

def len_scale_mcmc1(x_known, y_known, e_known, MC_progress, MC_plotting, labels, out_file_name):

    """
    Finds optimal log-length scales using MCMC + KS test loss.
    Works in log(length) space for smoother optimisation.
    """
    ...
    log_lower_bounds = np.log(lower_bounds)
    log_upper_bounds = np.log(upper_bounds)
    endpoints = np.column_stack((log_lower_bounds, log_upper_bounds))

    # Latin Hypercube initial positions in log space
    ...
    print("Beginning MCMC in log-space")
    ...
    with ctx.Pool(processes=num_cores) as pool:
        ...
        sampler = emcee.EnsembleSampler(
            nwalkers, ndim, ks_loss_logspace,
            args=(x_known, y_known, e_known, sigma_vals, log_lower_bounds, log_upper_bounds),
            backend=backend,
            pool=pool
        )
    ...
    if num_peaks > 1:
        ...
        cluster_labels = kmeans.fit_predict(samples)
        modes_log = kmeans.cluster_centers_

    else:
        print("Single mode surface")
        modes_log = np.mean(samples, axis=0, keepdims=True)

    print("Modes of surface (log space):")
    print(modes_log)
    print("Modes of surface (length space):")
    print(np.exp(modes_log))

    # Final local optimisation in log space
    def func_minimise(log_lengths):
        return -ks_loss_logspace(log_lengths, x_known, y_known, e_known, sigma_vals, log_lower_bounds, log_upper_bounds)

    bounds_log = [(np.log(1e-16), np.log(ub)) for ub in upper_bounds]
    score = np.inf
    best_log = None

    for j in range(len(modes_log)):
        result = minimize(func_minimise, modes_log[j], method="Nelder-Mead")
        if result.fun < score:
            best_log = result.x
            score = result.fun

    best = np.exp(best_log)  # back to length space

    print("Optimal length scale found (length space):")
    print(best)
